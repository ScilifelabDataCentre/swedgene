...
def make_alias_dictionary(ENA_fasta_file:str) -> dict[str, dict[str, str]]:
	"""
	...
	with open(fasta,'r') as file:
	    for line in file:
	        if line.startswith('>'):  											 # Find fasta headers in the multi-fasta
	        	if line.startswith('>ENA|'):
	        		ENA_fasta_header=re.split('>| ',line.rstrip())[1]  				 # Split on two delimiters (separated by the vertical bar regexp operator): the > and the first white space in the string
	        		NCBI_fasta_header=re.split('\\|| ', line.rstrip())[2]  			 # The double-escaped vertical bar - \\| - splits on the vertical bar in the string; the second vertical bar is a regexp operator that states that a second delimiter will follow: in this case the white space
	        		if re.search('contig: ', line):
		        		contig_name_fasta_header=re.split('contig: ',line.rstrip())[1] 	 # Splits on the substring 'contig: '
	        		else:
